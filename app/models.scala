...
case class Post(
  ...
  def prevNext = {       
    ...
    ).on("date" -> postedAt).as( opt('pos.is("prev")~>Post.on("")) ~ opt('pos.is("next")~>Post.on("")) ^^ flatten )
  }
}
...
object Post extends Magic[Post] {
  ...
  def allWithAuthorAndComments:List[(Post,User,List[Comment])] = {
    SQL(
        """ select * from Post p join User u on p.author_id = u.id left join Comment c on c.post_id = p.id order by p.postedAt desc """
    ).as( Post ~< User ~< Post.spanM( Comment ) ^^ flatten * )
  }
  def byIdWithAuthorAndComments(id: Long):Option[(Post,User,List[Comment])] = {
    SQL(
        """select * from Post p join User u on p.author_id = u.id left join Comment c on c.post_id = p.id where p.id = {id}"""
